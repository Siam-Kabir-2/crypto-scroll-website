...
function mountScrollWorld(container, config) {
  ...
  function read() {
    ...
    for (let i = 0; i < N; i++) {
      ...
      const c = copies[i];
      c.style.opacity = cop;
      // Desktop lays the copy out vertically centred via CSS `top:50%` +
      // `transform:translateY(-50%)`; since this inline style fully replaces
      // that stylesheet transform, the base -50% must be re-included here or
      // taller copy blocks (long titles/body/CTA rows) drift off the bottom
      // of the viewport. Mobile anchors the block to the bottom instead
      // (`transform:none` in the CSS breakpoint), so no base offset there.
      const centred = window.innerWidth > 860;
      const nudge = (0.5 - pr) * 4;
      c.style.transform = reduce ? (centred ? 'translateY(-50%)' : 'none')
        : (centred ? `translateY(calc(-50% + ${nudge}vh))` : `translateY(${nudge}vh)`);
